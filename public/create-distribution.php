...
$distribution = array();
while (count($distribution) < $values)
  {
  ...
  //Loop through normal distribution array to find where on the curve the value is found
  $stop = false;
  while (($distarraykey < 80) AND ($stop == false))
    ...
    if (($lowerdigit <= $digit) AND ($higherdigit >= $digit))
      {
      $stop = true;
  ...
  $normalisedigit = normalisevalue($digit,$lowerdigit,$higherdigit);
  $midsd = $highersd-$lowersd;
  $sdadjust = $normalisedigit*$midsd;
  $outputsd = $lowersd+$sdadjust;

  $sdoffset = $outputsd*$sd;
  $value = $mean+$sdoffset;
  array_push($distribution,$value);
  }
?>
